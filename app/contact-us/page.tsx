            ...
            <div className="bg-white dark:bg-[#060B1A] rounded-[48px] p-8 md:p-12 border border-slate-100 dark:border-white/5 shadow-2xl">
              {!submitted ? (
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <input required type="tel" placeholder="Phone Number" className={inputStyle} onChange={e => setFormData({...formData, phone: e.target.value})} />
                    
                    {/* FIXED DROPDOWN SECTION */}
                    <div className="relative group">
                      <select 
                        required 
                        style={{ colorScheme: 'light dark' }} // Browser native popup theme fix
                        className={`${inputStyle} appearance-none cursor-pointer pr-12`} 
                        onChange={e => setFormData({...formData, service: e.target.value})}
                      >
                        <option value="" disabled selected className="bg-white dark:bg-[#0B0F1A]">Select Service</option>
                  ...
                  <textarea required placeholder="Tell us about your startup needs..." className={inputStyle + " h-32 resize-none"} onChange={e => setFormData({...formData, message: e.target.value})}></textarea>
